""" 
This program can store items into normal container, multi-container and magic container, and list the inside items.
"""
import copy
...
class Item:
    ...
    def __init__(self, name: str = "", weight: int = 0) -> str:
        """
...
class Container(Item):
    ...
    def add_item(self, item: Item) -> None:
        ...
        item_weight = 0
        if isinstance(item, MultiContainer):
            #the MultiContainer's default weight is set as 0, 
            #so call this method here to add all compartments' weight up.
            item_weight = item.calculate_empty_weight()
        else:
            item_weight = item.weight

        #Base case
        if item_weight <= self.get_remaining_capacity():
            self.items.append(item)
        else:
            for inner_item in self.items:
                #When it contains magic container, 
                #the items inside magic container have no effect on the outermost container's capacity.
                if isinstance(inner_item, MagicContainer):
                    try:
                        #so try add item into magic container here. 
                        inner_item.add_item(item)
                        return
                    except OutOfCapacity:
                        continue
            #If all the inner magic containers are full or outer container doesn't have any magic container inside, 
            #then raise an OutOfCapacity exception.
            raise OutOfCapacity
...
class MagicContainer(Container):
    """
    A representation of a MagicContainer.
    """

    def calculate_total_weight(self) -> int:
        """
        Calculate a magic container's total weight.
        """

        return self.weight

#MagicMultiContainer inherits from MultiContainer and MagicContainer simultaneously
#and we make the MultiContainer as the first parameter
#so that MagicMultiContainer will inherit the __str__ method from MultiContainer instead of MagicContainer.
class MagicMultiContainer(MultiContainer, MagicContainer):
    """
    A representation of a MagicMultiContainer.
    """

    def calculate_total_weight(self) -> int:
        """
        Calculate a magic multi container's total weight.
        """

        #Invoke this method in MultiContainer, super refers to the Multicontainer class.
        #The total weight of a magicmulticontainer equals to the total empty weight of its compartments.
        return super().calculate_empty_weight()
...
def read_csv(filename: str) -> None:
    ...
    with open(filename, "r") as ref_file:
        ...
        for line in sorted(lines[1:]):
            ...
            if filename == "items.csv":
            ...
            elif filename == "multi_containers.csv":
                compartments_list = []
                ...
                #and add this multiContainer to the items_list.
                items_list.append(MultiContainer(name = vals[0], items = compartments_list))
            elif filename == "magic_containers.csv":
                #copy from the same-name container so that its instance variable 
                #can be used to create an instance of magic container.
                container = copy_item(vals[1].strip())
                items_list.append(MagicContainer(name = vals[0].strip(), weight = container.weight, capacity = container.capacity))
            else:
                multi_container = copy_item(vals[1].strip())
                items_list.append(MagicMultiContainer(name = vals[0].strip(),items = multi_container.items))
...
if __name__ == '__main__':
    ...
    items_list = []
    files_list = ["items.csv", "containers.csv", "multi_containers.csv", "magic_containers.csv", "magic_multi_containers.csv"]
